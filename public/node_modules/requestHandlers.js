    ...
    if (messObj.email != null) {
	  ...
	  mdb.action('getIdByUname', messObj.recip, function(err, results) {
	    if (results === null) {
		  ...
		  txt = txt + "\n\n" + msgs.forgotpwp1 + 
		          '\n\n  http://localhost:11329/MHCreatives/rdata?dat=' + results._id +
                  "\n\n" + msgs.forgotpwp2 + "\n\n" + config.emailAdmin +
                  "\n\n" + msgs.forgotpwend;
		  var subj = "Your Reset Request..."
          nmail.send(messObj.recip, subj, txt, response);				  
...
function register(request, response) {
  ...
  request.on('end', function () {
    ...
    if (nMember.uid != null) {
      console.log('new member = ' + nMember.name.fname + " " + nMember.name.lname);
      console.log('member obj = ' + JSON.stringify(nMember));	  
      //var cc = json['col'];
	  mdb.action('registerMember', nMember, function(err, results) {
	    if (results === null) {
		  response.writeHead(200, {"Content-Type": "application/json"});
		  response.write({"error":"record not inserted"});
		  console.log("content-->: " + rdataS);
		  response.end();
		} else {
	      /*txt = "Hello " + results.name.fname + ' ' + results.name.lname;
		  ...
		  var subj = "Your Reset Request..."*/
          //nmail.send(messObj.recip, subj, txt, response);				  
	      console.log("User ID = " + results[0]._id);
		}
	  });
    }
  });	
...
function updateDB(request, response) {
  ...
  request.on('end', function () {
	...
	//qdata = {id:(session.getSession(data.sid)).member_id,prop:data.prop,nuval:data.nuval};
    mdb.action('updateMemeber', qdata, function(err, results) {
		if (err) {
...
//----------------------------------------------------
  ...
  busboy.on('file', function(fieldname, file, filename, encoding, mimetype) {
    console.log("fieldname : " + fieldname + "; file : " + file + "; filename : " + filename);
...
function reset(request, response) {
  ...
  mdb.action('getById', _id, function(err, results) {			  
	...
	var instream = fs.createReadStream('./templates/rdata.tmpl');
