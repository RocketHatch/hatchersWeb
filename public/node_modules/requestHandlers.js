    ...
    if (messObj.email != null) {
	  ...
	  mdb.action('getIdByUname', messObj.recip, function(err, results) {
	    if (results === null) {
		  ...
		  txt = txt + "\n\n" + msgs.forgotpwp1 + 
		          '\n\n  http://'+config.host+':'+config.port+'/MHCreatives/rdata?dat=' + results._id +
                  "\n\n" + msgs.forgotpwp2 + "\n\n" + config.emailAdmin +
                  "\n\n" + msgs.messageend;
		  var subj = "reset";
		  var txts = [txt, ret];
...
function register(request, response) {
  ...
  request.on('end', function () {
    ...
    if (nMember.uid != null) {	  
	  mdb.action('registerMember', nMember, response);
    }
  });
}
...
function confirm(request, response) {
  ...
  mdb.action('getById', _id, function(err, results) {			  
	...
	var readline = require('readline');
	var stream = require('stream');
	var instream = fs.createReadStream('./templates/ndata.tmpl');
	var outstream = new stream;
	var rl = readline.createInterface(instream, outstream);
