...
function insertMember(data, response) {
  var oriDate = new Date(2014,4,4);
  var collection = db.collection("members");
  var ret = null;
  var member = data.member;
  //var sig = data.signature;
  //console.log("sig got to dbactions: " + sig);  
  getIdByUname(member.uid, function(err, result) {
    assert.equal(null, err);
	console.log("result = " + result);
	if (!result) {
      collection.insert(member, {w:1}, function(err, results) {
        assert.equal(null, err);
	    console.log("insert new id = " + results[0]._id);
	    var nid = "" + results[0]._id;
	    collection.update({_id:results[0]._id}, {$set: {dateCreate:new Date()}}, function(err,result) { 
          assert.equal(null, err);
		  ret = {"message":"check your email to confirm registration", "nid":nid};
		  txt = "Hello " + results[0].name.fname + ' ' + results[0].name.lname;
		  txt = txt + "\n\n" + msgs.newreg1 + 
		          '\n\n  http://'+process.env.HOST+'/ndata?dat=' + nid +
                  "\n\n" + msgs.newreg2 + "\n\n" + 'hatchers@rockethatch' +
                  "\n\n" + msgs.messageend;
		  var subj = "newReg"
		  var txts = [txt,ret]
          nmail.send(results[0].uid, subj, txts, response);  
        });		
	  });
    } else {
	  ret = {"message":"A member with this email address aleady exists."};
...
function updateMember(data, response) {
  ...
  var set = {};
  set[data.prop] = (data.prop).substring(0,4) === "date" ? new Date(data.nuval) : data.nuval;
...
function getCollection(col, response) {
  ...
  var q = (col === 'skills') ? null : {dateExpire: {$gte : new Date()}};
